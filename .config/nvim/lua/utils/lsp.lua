...
local function get_format_filter(args)
	...
	-- We assume there can be only two LSP servers providing formatting: null-ls and another one (from lspconfig).
...
function M.setup_on_attach()
	-- This function is supposed to be called in the config of both lspconfig and null-ls.
	-- Nevertheless, this autocommand will not be duplicated due to `clear=true`.
	vim.api.nvim_create_autocmd("LspAttach", {
		group = vim.api.nvim_create_augroup("my-lsp-attach", { clear = true }),
		-- For every buffer, this callback will be executed multiple times, once for each attached LSP client.
		-- Thus, we need extra care to avoid conflicts between LSP clients.
		-- Note: Setting keymaps multiple times is not harmful.
		callback = function(args)
			...
			vim.api.nvim_create_autocmd("BufWritePre", {
				...
				buffer = args.buf,
				callback = function()
					vim.lsp.buf.format({ async = false, filter = get_format_filter(args) })
				end,
			})
		end,
	})
